...
public class TranslateMove : MonoBehaviour {
	...
	void Update () {
        if(m_fowardMove)
            transform.Translate(transform.forward * m_power * Time.deltaTime);
        if (m_rightMove)
            transform.Translate(transform.right * m_power * Time.deltaTime);
        if (m_upMove)
            transform.Translate(transform.up * m_power * Time.deltaTime);
