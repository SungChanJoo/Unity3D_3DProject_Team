    ...
    public class PrefabChildManagerEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (prefabManager.showSetup)
            {
                ...
                prefabManager.showFullInspector =
                    EditorGUILayout.Toggle("Show Full Inspector", prefabManager.showFullInspector);
                prefabManager.instantiatePrefabsAsAdded =
                    EditorGUILayout.Toggle("Instantiate Prefabs when Added to Group",
                        prefabManager.instantiatePrefabsAsAdded);
                prefabManager.onlyOneGroupActivePerType = EditorGUILayout.Toggle("Only one group active per type",
                    prefabManager.onlyOneGroupActivePerType);
                prefabManager.unpackPrefabs =
                    EditorGUILayout.Toggle("Unpack Prefabs when Instantiated", prefabManager.unpackPrefabs);

                EditorGUILayout.Space();
                if (prefabManager.showHelpBoxes)
                {
                    EditorGUILayout.HelpBox(
                        "Use the option below to set all InGameObject values to null. This is useful " +
                ...
                {
                    EditorGUILayout.HelpBox(
                        "If you've copied another objects data or added the component from another object " +
                ...
                {
                    EditorGUILayout.HelpBox(
                        "Press \"C\" to copy a group to the clipboard. Then, at the bottom of the group list on " +
                        ...
                        "useful when copying groups from one character to another, so long as they share the same transform names.",
                        MessageType.None);
                }
                /*
            ...
            prefabManager.showPrefabGroups = EditorGUILayout.Foldout(prefabManager.showPrefabGroups,
                prefabManager.prefabGroups.Count + " Prefab Groups");
                ...
                for (int g = 0; g < prefabManager.prefabGroups.Count; g++)
                {
                    ...
                    if (GUILayout.Button("X", GUILayout.Width(25)))
                    {
                                ...
                                "?", "Yes", "Cancel"))
                        {
                            ...
                            {
                                RemoveObject(prefabManager, prefabManager.prefabGroups[g],
                                    prefabManager.prefabGroups[g].prefabObjects[i], i);
                            }
                    ...
                    else
                    {
                        ...
                        prefabManager.prefabGroups[g].name =
                            EditorGUILayout.TextField(prefabManager.prefabGroups[g].name, GUILayout.Width(150));

                        EditorGUILayout.LabelField("Type", GUILayout.Width(50));
                        Undo.RecordObject(prefabManager, "Change Prefab Group Type");
                        prefabManager.prefabGroups[g].groupType =
                            EditorGUILayout.TextField(prefabManager.prefabGroups[g].groupType, GUILayout.Width(100));

                        EditorGUILayout.LabelField("Default", GUILayout.Width(50));
                        Undo.RecordObject(prefabManager, "Change Prefab Group Default");
                        prefabManager.prefabGroups[g].isDefault =
                            EditorGUILayout.Toggle(prefabManager.prefabGroups[g].isDefault);
                        ...
                        if (prefabManager.prefabGroups[g].showPrefabs)
                        {
                            ...
                            if (GUILayout.Button("Objects (" + prefabManager.prefabGroups[g].prefabObjects.Count + ")",
                                    GUILayout.Width(80)))
                            {
                        ...
                        if (prefabManager.prefabGroups[g].showPrefabs)
                        {
                            ...
                            for (int i = 0; i < prefabManager.prefabGroups[g].prefabObjects.Count; i++)
                            {
                                ...
                                else
                                {
                                    ...
                                    prefabObject.prefab = EditorGUILayout.ObjectField(prefabObject.prefab,
                                        typeof(GameObject), !prefabObject.isPrefab) as GameObject;
                                    ...
                                    if (prefabObject.isPrefab)
                                    {
                                        ...
                                        prefabObject.parentTransform =
                                            EditorGUILayout.ObjectField(prefabObject.parentTransform, typeof(Transform),
                                                true) as Transform;
                                        if (oldTransformObject != prefabObject.parentTransform)
                                        {
                                            ...
                                            if (!prefabObject.parentTransform.IsChildOf(prefabManager.thisTransform))
                                            {
                                                ...
                                                Debug.LogError(
                                                    "Error: Transform must be the parent transform or a child of " +
                                                    prefabManager.thisTransform.name);
                                            }
                                            else
                                            {
                                                Event e = Event.current;
                            ...
                            prefabManager.prefabGroups[g].newPrefab =
                                EditorGUILayout.ObjectField("Add Prefab or Child Object to Group",
                                    prefabManager.prefabGroups[g].newPrefab, typeof(GameObject), true) as GameObject;
                            if (prefabManager.prefabGroups[g].newPrefab)
                            {

                                if (prefabManager.prefabGroups[g].newPrefab.transform
                                    .IsChildOf(prefabManager.transform))
                                    ...
                                    Debug.LogError("Error: " + prefabManager.prefabGroups[g].newPrefab.name +
            ...
            {
                if (GUILayout.Button(new GUIContent("Paste group from clipboard",
                        "Paste the copied group into this as a new Prefab Group.")))
                {
        ...
        private void RemoveObject(PrefabChildManager prefabManager, PrefabGroup prefabGroup, PrefabObject prefabObject,
            int i)
        {
